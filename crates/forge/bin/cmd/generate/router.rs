...
use foundry_compilers::{artifacts::{output_selection::OutputSelection, ConfigurableContractArtifact}, info::ContractInfo, CompilerSettings};
use foundry_config::{
    figment::{
        value::{Dict, Map},
...
impl GenerateRouterArgs {
    pub fn run(self) -> Result<()> {
        ...
        let mut targets: HashMap<String, Option<(ContractInfo, ConfigurableContractArtifact)>> = HashMap::new();
        ...
        for (path, name, info) in output.into_artifacts_with_files()  {
            for (module_name, module_info) in &mut targets {
                ...
                if let Some(target_path) = &target.path {
                    ...
                    } else if let Ok(resolved_path) = project.paths.resolve_import(project.root(), Path::new(&target_path)) {
...
fn build_router(
        ...
        for function_set in abi.functions.iter() {
            for function in function_set.1.iter() {
                if functions.contains_key(&function.selector()) {
                    return Err(eyre::eyre!(format!("Duplicate selector found {}", function.signature())));
                }
    ...
    let router_content = include_str!("../../../assets/generated/RouterTemplate.sol");
